...
export class Camera implements Invalidable {
	...
	public adaptToCanvas(canvas: HTMLCanvasElement) {
		const { left: minX, top: minY } = canvas.getBoundingClientRect();
		...
		const horizontalPPU = this.planeSide / width;
		const verticalPPU = this.planeSide / height;
	...
	public clientCoordToCamera(clX: number, clY: number): [number, number] {
		...
		return [(clX - minY) * this.DPR, (clY - minX) * this.DPR];
	}
	...
	private resizeViewport(center?: [number, number]) {
		this.viewport.resize(
			boundsWidth(this.cameraBounds) * this.curUPP,
			boundsHeight(this.cameraBounds) * this.curUPP,
			center,
		);
	}
}
